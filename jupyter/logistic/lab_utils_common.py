...
def plt_sigmoid(x0, x1, x, y,  w, b, ax):
    ...
    m = 30
    x_hat = np.linspace(x0, x1, m)
    y_hat  = np.zeros_like(x_hat, dtype=float)
    for i in range(m):
       z =  w*x_hat[i] + b          # protect against overflow
       y_hat[i] = 1.0/(1.0+np.exp(-z))
    ax.plot(x_hat, y_hat, color=dlc["dlblue"],
                                     label="y = sigmoid(z)")
