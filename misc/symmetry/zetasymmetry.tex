...
\label{eq:mean}
\end{equation}
Equation \ref{eq:mean} was explicitly called out for Gram points ($\phi = 0$ and $\phi = \pi$) in Ref.~\cite{Titchmarsh 1986}. The proof of Ref.~\cite{Titchmarsh 1986} can be extended to cover Equation \ref{eq:mean}. Our other two conjectures are that $p_{\phi}(y)$ satisfies the anti-symmetry condition
\begin{equation}
