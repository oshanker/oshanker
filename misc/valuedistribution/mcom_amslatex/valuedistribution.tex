...
The large values of  kurtosis show that the distributions tend to have heavy tails, or outliers. Because of these heavy tails, the higher moments of the distribution function are likely to diverge logarithmically (we already saw that the mean is finite and well-behaved). The distributions  also have large skew, except for $\phi = \pi/2$ and $\phi = 3\pi/2$, where the distributions are symmetric. 
...
$T_1 \rightarrow \infty, T_2 \rightarrow \infty,  T_1-T_2 \rightarrow \infty,$ however
\begin{equation}
T_2 - T_1  \ll T_1. 
\label{eq:interval}
\end{equation}
...
It is not known whether the limiting probability distribution exists for $Z(t)$ at generalized Gram points. However, we can get some insight from the studies of Kalpokas and Steuding \cite{kalpokas 2009}. They show that
\begin{equation}
\sum_{0 < t < T, \zeta(\frac{1}{2}+it) \in e^{i\phi_1}\mathbb{R}} \zeta(\frac{1}{2}+it) = (2e^{i\phi_1}\cos(\phi))\frac{T}{2\pi}\ln (\frac{T}{2\pi}) + O_\epsilon(T^{(\frac{1}{2}+\epsilon}).
\label{eq:kalpokas1}
\end{equation}
...
\begin{equation}
\sum_{0 < t < T, \zeta(\frac{1}{2}+it) \in e^{i\phi_1}\mathbb{R}} |\zeta(\frac{1}{2}+it)|^2 = \frac{T}{2\pi}(\ln (\frac{T}{2\pi}))^2 + (2\gamma+2\cos(2\phi))\frac{T}{2\pi}\ln (\frac{T}{2\pi}) + \frac{T}{2\pi} + O_\epsilon(T^{(\frac{1}{2}+\epsilon}).
\label{eq:kalpokas2}
\end{equation}
From Equation~\ref{eq:kalpokas2} and the cardinality of our sample space it seems that higher order moments of the value distribution diverge logarithmically. Thus, it seems likely that a limiting value distribution exists, it has a finite well-defined mean, and higher moments diverge logarithmically. Distributions which do not have all moments defined are used in applications, for example, the Cauchy-Lorentz distribution~\cite{feller}.
