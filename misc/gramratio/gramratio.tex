...
\hline
\end{array}\)
\caption{Counts of Gram intervals that contain $m$ zeros for three samples at $t=10^{12}$, $t=10^{15}$  and $t=10^{28}$ respectively, and the  expected values using Odlyzko's prediction.} \label{tab:intervalzeros}
\end{table}
...
Table~\ref{tab:intervalzeros} shows the counts of Gram intervals that contain $m$ zeros for three samples at $t=10^{12}$, $t=10^{15}$  and $t=10^{28}$ respectively, and the  expected values using Odlyzko's prediction. The first three rows of the table are based on Hiary. The last row is from Ref.~\cite{Odlyzko 1992}.
