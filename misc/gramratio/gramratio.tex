...
Table~\ref{tab:rosserrandom}  shows the ratio of $Type~II/Type~I$ counts when we displace the Gram points by $k\delta$, where $\delta$ is the Gram interval at height $t$ (Eq.~\ref{eq:spacing}). For our sample space $\delta$ is essentially constant. The table includes only regular Gram blocks, since $Type~II$ and $Type~I$ Gram blocks are defined only for regular Gram blocks.  The statistics are from $10$ million Gram intervals at $t=10^{28}$. 
...
Table~\ref{tab:vrr}  (based on data in \cite{gourdon}) shows the  symmetry under reversal of the zero patterns for different types violations of Rosser's rule. 
...
the Gram points. These conjectures are most likely related to the symmetry  of the rotated zeta function (Hardy's function $Z(t)$ ) under the transformation $t \rightarrow -t$. 
Let $p_{odd}(y)$ be the probability distribution function for $Z(t)$ at odd Gram points, i.e., 
\begin{equation}
\int\limits_{a}^{b} p_{odd}(y)dy
\label{eq:pdfodd}
\end{equation}
is the probability that $a<Z(t)<b$ when we consider the values of $Z(t)$ for a large number of odd Gram points in the sample space. Similarly, let $p_{even}(y)$ be the probability distribution function for $Z(t)$ at even Gram points. Then we find empirically that
